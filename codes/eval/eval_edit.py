...
def process(model, ddim_sampler, item_dict, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, strength, scale, seed, eta,input_dir):
    with torch.no_grad():
        ...
        # get masked_x
        input_dir=os.path.join(input_dir,item_dict['img_name'].split(".")[0]+"_0.jpg")
        
        ref_img = cv2.imread(input_dir)
        
        masked_img = ((ref_img.astype(np.float32) / 127.5) - 1.0)*(1-hint)
        output=(masked_img+1)*127.5
                # 确保像素值在 0 到 255 的范围内
        output = np.clip(output, 0, 255)

        # 将数据类型转换为 np.uint8
        output = output.astype(np.uint8)
        cv2.imwrite("/root/autodl-tmp/debug/"+item_dict['img_name'],output)
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(data_list)), desc='generator'):
        ...
        if os.path.exists(os.path.join(args.output_dir, img_name)):
            continue
        results = process(model, ddim_sampler, item_dict, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, strength, scale, seed, eta,args.input_dir)
        times += [results.pop()]
        if i == total:
            print(times)
            times = times[1:]
